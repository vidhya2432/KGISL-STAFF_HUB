      ...
      <Tabs defaultValue="syllabus">
        ...
        <TabsContent value="syllabus" className="space-y-6 mt-6">
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  ...
                  <Button variant="outline" className="w-full rounded-full py-6 text-[#1d1d1f] font-bold" onClick={() => syllabusFileInputRef.current?.click()} disabled={isExtracting}>{isExtracting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Upload className="mr-2 h-4 w-4" />} Upload Syllabus</Button>
                  <Textarea placeholder="Paste syllabus text here..." className="min-h-[120px] rounded-2xl text-[#1d1d1f] font-medium" value={syllabusInput} onChange={(e) => setSyllabusInput(e.target.value)} />
                </div>
                <div className="space-y-6">
                  <Label className="text-sm font-bold uppercase tracking-wider text-[#86868b]">Step 2: Assignment Details</Label>
                  <Select value={assignmentType} onValueChange={setAssignmentType}>
                    <SelectTrigger className="rounded-full py-6 text-[#1d1d1f] font-bold"><SelectValue placeholder="Select type..." /></SelectTrigger>
                    <SelectContent className="rounded-2xl bg-white">
                      <SelectItem value="Case Study" className="focus:bg-[#f5f5f7] focus:text-[#1d1d1f] text-[#1d1d1f] font-bold">Case Study</SelectItem>
                      <SelectItem value="Research Paper" className="focus:bg-[#f5f5f7] focus:text-[#1d1d1f] text-[#1d1d1f] font-bold">Research Paper</SelectItem>
                      <SelectItem value="Problem Set" className="focus:bg-[#f5f5f7] focus:text-[#1d1d1f] text-[#1d1d1f] font-bold">Problem Set</SelectItem>
                      <SelectItem value="Essay" className="focus:bg-[#f5f5f7] focus:text-[#1d1d1f] text-[#1d1d1f] font-bold">Essay</SelectItem>
                      <SelectItem value="Practical Lab Report" className="focus:bg-[#f5f5f7] focus:text-[#1d1d1f] text-[#1d1d1f] font-bold">Practical Lab Report</SelectItem>
                    </SelectContent>
                  </Select>
                  <Label className="text-sm font-bold uppercase tracking-wider text-[#86868b]">Step 3: Mode</Label>
                  <div className="flex gap-4"><Button variant={assignmentGrouping === 'Individual' ? 'default' : 'outline'} className="flex-1 rounded-full py-6 font-bold" onClick={() => setAssignmentGrouping('Individual')}><User className="mr-2 h-4 w-4" /> Individual</Button><Button variant={assignmentGrouping === 'Team' ? 'default' : 'outline'} className="flex-1 rounded-full py-6 font-bold" onClick={() => setAssignmentGrouping('Team')}><Users2 className="mr-2 h-4 w-4" /> Team</Button></div>
                  {assignmentGrouping === 'Team' && <div className="space-y-2 animate-in fade-in slide-in-from-top-1"><Label className="text-sm font-bold uppercase tracking-wider text-[#86868b]">Students per Team</Label><Input type="number" min={2} max={15} value={teamSize} onChange={(e) => setTeamSize(parseInt(e.target.value))} className="rounded-full py-6 text-[#1d1d1f] font-bold" /></div>}
                </div>
              </div>
          ...
          {!hideGenerationResult && suggestionState.data?.suggestedAssignments && (
            <Card className="rounded-[32px] border-[#0066cc]/30 shadow-lg animate-in zoom-in-95 duration-200"><CardHeader className="flex flex-row items-center justify-between"><div><CardTitle className="text-[#0066cc] flex items-center gap-2 text-2xl font-bold"><ClipboardList className="h-6 w-6" /> Generated Topic Distribution</CardTitle></div><Button onClick={finalizeAssignment} className="apple-button-primary font-bold"><CheckCircle2 className="mr-2 h-4 w-4" /> Confirm & Save to Class</Button></CardHeader><CardContent><div className="rounded-2xl border border-[#d2d2d7] max-h-[300px] overflow-auto"><Table><TableHeader><TableRow className="bg-[#f5f5f7]"><TableHead className="font-bold text-[#1d1d1f]">Index</TableHead><TableHead className="font-bold text-[#1d1d1f]">Suggested Topic</TableHead></TableRow></TableHeader><TableBody>{suggestionState.data.suggestedAssignments.map((topic: string, i: number) => (<TableRow key={i}><TableCell className="font-mono text-xs text-[#1d1d1f] font-bold">{i + 1}</TableCell><TableCell className="text-sm text-[#1d1d1f] font-bold">{topic}</TableCell></TableRow>))}</TableBody></Table></div></CardContent></Card>
          )}
          {assignments.length > 0 && (
            <div className="space-y-6 pt-12"><h3 className="text-2xl font-semibold tracking-tight flex items-center gap-2 text-[#1d1d1f]"><Layers className="h-5 w-5" /> Active Class Assignments</h3><div className="grid gap-4">{assignments.map(a => (<Card key={a.id} className="hover:bg-white transition-colors border-[#d2d2d7] rounded-3xl shadow-sm border-l-4 border-l-[#0066cc]"><CardContent className="p-6 flex items-center justify-between"><div className="flex items-center gap-6"><div className="bg-[#0066cc]/10 p-3 rounded-2xl"><FileSpreadsheet className="h-6 w-6 text-[#0066cc]" /></div><div><p className="font-bold text-lg text-[#1d1d1f]">{a.title}</p><div className="flex gap-2 items-center mt-1"><Badge variant="outline" className="text-[10px] h-4 rounded-md font-bold text-[#1d1d1f]">{a.type}</Badge><span className="text-xs text-[#86868b] font-bold">{(a.mappings || []).length} {a.grouping} Topics</span></div></div></div><div className="flex gap-4"><Button variant="ghost" size="sm" onClick={() => setActiveAssignmentId(a.id)} className={cn("rounded-full px-6 font-bold", activeAssignmentId === a.id ? 'bg-[#f5f5f7] text-[#1d1d1f]' : 'text-[#86868b]')}>{activeAssignmentId === a.id ? 'Active' : 'Select'}</Button><Button variant="outline" size="sm" className="rounded-full px-6 font-bold text-[#1d1d1f]" onClick={() => downloadAssignmentSheet(a)}><Download className="h-4 w-4 mr-2" /> Download Sheet</Button><Button variant="ghost" size="icon" onClick={() => db && deleteDoc(doc(db, 'classes', classId, 'assignments', a.id))} className="text-[#ff3b30] hover:text-[#ff3b30] hover:bg-[#ff3b30]/10 rounded-full"><Trash2 className="h-4 w-4" /></Button></div></CardContent></Card>))}</div></div>
          )}
        </TabsContent>

        <TabsContent value="roster" className="space-y-6 mt-6">
          <Card className="rounded-[32px] border-[#d2d2d7] shadow-sm"><CardHeader className="flex flex-row items-center justify-between space-y-0"><div><CardTitle className="text-2xl font-semibold text-[#1d1d1f]">Class Roster</CardTitle><CardDescription className="text-base font-medium">{activeAssignment ? `Showing topics for: ${activeAssignment.title}` : 'Add students or upload a roster list.'}</CardDescription></div><div className="flex gap-3"><Button variant="outline" size="sm" onClick={clearRoster} className="text-[#ff3b30] border-[#ff3b30] hover:bg-[#ff3b30]/10 rounded-full px-6 font-bold"><Eraser className="h-4 w-4 mr-2" /> Clear List</Button><input type="file" accept=".xlsx,.xls,.csv,.pdf,image/*" className="hidden" ref={rosterFileInputRef} onChange={handleRosterUpload} /><Button variant="secondary" size="sm" className="rounded-full px-6 text-[#1d1d1f] font-bold" onClick={() => rosterFileInputRef.current?.click()} disabled={isRosterUploading}>{isRosterUploading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Upload className="h-4 w-4 mr-2" />} Import Roster</Button></div></CardHeader><CardContent className="space-y-6"><div className="flex gap-4"><Input placeholder="Roll No" className="w-[180px] rounded-full text-[#1d1d1f] font-bold" value={newStudentRoll} onChange={e => setNewStudentRoll(e.target.value)} /><Input placeholder="Full Name" className="flex-1 rounded-full text-[#1d1d1f] font-bold" value={newStudentName} onChange={e => setNewStudentName(e.target.value)} /><Button variant="outline" className="rounded-full w-12 h-12 text-[#0066cc] border-[#0066cc] hover:bg-[#0066cc]/10" onClick={handleAddStudent}><Plus className="h-5 w-5" /></Button></div><div className="rounded-2xl border border-[#d2d2d7] max-h-[500px] overflow-auto shadow-inner"><Table><TableHeader><TableRow className="bg-[#f5f5f7]"><TableHead className="font-bold text-[#1d1d1f]">Roll No / ID</TableHead><TableHead className="font-bold text-[#1d1d1f]">Name</TableHead>{activeAssignment && <TableHead className="font-bold text-[#1d1d1f]">Assigned Topic</TableHead>}<TableHead className="text-right font-bold text-[#1d1d1f]">Action</TableHead></TableRow></TableHeader><TableBody>{roster.map(s => { const mapping = activeAssignment?.mappings?.find(m => m.assigneeId.includes(s.roll)); return (<TableRow key={s.id} className="hover:bg-secondary/20"><TableCell className="font-mono text-xs text-[#1d1d1f] font-bold">{s.roll}</TableCell><TableCell className="font-bold text-[#1d1d1f]">{s.name}</TableCell>{activeAssignment && (<TableCell className="text-sm italic text-[#86868b] max-w-[300px] truncate font-bold" title={mapping?.topic}>{mapping?.topic || 'TBD'}</TableCell>)}<TableCell className="text-right"><Button variant="ghost" size="icon" className="rounded-full hover:bg-[#ff3b30]/10" onClick={() => db && deleteDoc(doc(db, 'classes', classId, 'students', s.id))}><Trash2 className="h-4 w-4 text-[#ff3b30]" /></Button></TableCell></TableRow>);})}{roster.length === 0 && <TableRow><TableCell colSpan={activeAssignment ? 4 : 3} className="text-center py-24 text-[#86868b] italic font-bold">Roster is empty. Import a list to get started.</TableCell></TableRow>}</TableBody></Table></div></CardContent></Card>
        </TabsContent>

        <TabsContent value="submissions" className="space-y-6 mt-6">
          <Card className="rounded-[32px] border-[#d2d2d7] shadow-sm"><CardHeader className="flex flex-row items-center justify-between"><div><CardTitle className="text-2xl font-semibold text-[#1d1d1f]">Grading Hub</CardTitle><CardDescription className="text-base font-medium">Track submissions and AI analysis.</CardDescription></div><div className="flex gap-4">
            <Select value={activeAssignmentId} onValueChange={setActiveAssignmentId}>
              <SelectTrigger className="w-[280px] rounded-full py-6 text-[#1d1d1f] font-bold"><SelectValue placeholder="Select Assignment" /></SelectTrigger>
              <SelectContent className="rounded-2xl bg-white">
                {assignments.map(a => <SelectItem key={a.id} value={a.id} className="focus:bg-[#f5f5f7] focus:text-[#1d1d1f] text-[#1d1d1f] font-bold">{a.title}</SelectItem>)}
              </SelectContent>
            </Select>
            <Dialog open={isDriveDialogOpen} onOpenChange={setIsDriveDialogOpen}><DialogTrigger asChild><Button className="apple-button-primary font-bold" disabled={!activeAssignmentId || roster.length === 0}>{isSyncing ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <CloudDownload className="mr-2 h-4 w-4" />} Analyze Drive</Button></DialogTrigger><DialogContent className="rounded-[32px] bg-white border-[#d2d2d7] shadow-2xl"><DialogHeader><DialogTitle className="text-[#1d1d1f] font-bold text-xl">Connect Submission Folder</DialogTitle></DialogHeader><div className="py-6 space-y-4"><div className="space-y-2"><Label className="text-sm font-bold uppercase tracking-wider text-[#86868b]">Google Drive Link</Label><Input placeholder="https://drive.google.com/..." className="rounded-full py-6 text-[#1d1d1f] font-bold" value={driveLink} onChange={e => setDriveLink(e.target.value)} /></div></div><DialogFooter><Button onClick={handleSyncDrive} disabled={!driveLink} className="apple-button-primary w-full font-bold py-6">Start Sync</Button></DialogFooter></DialogContent></Dialog></div></CardHeader><CardContent>{!activeAssignmentId ? (<div className="flex flex-col items-center justify-center p-24 border-2 border-dashed rounded-[32px] bg-[#f5f5f7]/50"><AlertCircle className="h-16 w-16 text-[#86868b] opacity-20 mb-6" /><p className="text-lg text-[#86868b] font-medium">Select an assignment to begin grading.</p></div>) : (<><div className="mb-6 flex flex-wrap gap-3"><Badge variant="secondary" className="flex gap-2 items-center px-4 py-1.5 rounded-full bg-[#f5f5f7] border-[#d2d2d7] text-[#1d1d1f] font-bold"><Layers className="h-3 w-3" /> {activeAssignment?.type}</Badge><Badge variant="secondary" className="flex gap-2 items-center px-4 py-1.5 rounded-full bg-[#f5f5f7] border-[#d2d2d7] text-[#1d1d1f] font-bold">{activeAssignment?.grouping === 'Individual' ? <User className="h-3 w-3" /> : <Users2 className="h-3 w-3" />} {activeAssignment?.grouping}</Badge><Badge variant="outline" className="px-4 py-1.5 rounded-full border-[#d2d2d7] text-[#1d1d1f] font-bold">{activeSubmissions.length} Total Entries</Badge></div><div className="rounded-[24px] border border-[#d2d2d7] overflow-hidden shadow-sm"><Table><TableHeader><TableRow className="bg-[#f5f5f7]"><TableHead className="font-bold text-[#1d1d1f]">{activeAssignment?.grouping === 'Individual' ? 'Student' : 'Team / Members'}</TableHead><TableHead className="font-bold text-[#1d1d1f]">Status</TableHead><TableHead className="font-bold text-[#1d1d1f]">AI Plagiarism</TableHead><TableHead className="text-right font-bold text-[#1d1d1f]">Grade & Feedback</TableHead></TableRow></TableHeader><TableBody>{activeSubmissions.map(sub => (<TableRow key={sub.studentId} className="hover:bg-secondary/20 transition-colors"><TableCell><div><p className="font-bold text-[#1d1d1f]">{sub.studentName}</p><p className="text-xs text-[#86868b] font-mono font-bold">{sub.rollNumber}</p></div></TableCell><TableCell><Badge className="rounded-full px-3 font-bold" variant={sub.status === 'Submitted' ? 'secondary' : sub.status === 'Late' ? 'destructive' : 'outline'}>{sub.status}</Badge></TableCell><TableCell>{sub.plagiarismScore !== null && <div className="flex items-center gap-3"><Progress value={sub.plagiarismScore * 100} className="w-[80px] h-1.5" /><span className="text-[10px] font-bold text-[#1d1d1f]">{(sub.plagiarismScore * 100).toFixed(0)}%</span></div>}</TableCell><TableCell className="text-right"><div className="flex items-center justify-end gap-3">{sub.marks && <span className="font-bold text-[#0066cc] text-lg">{sub.marks}%</span>}<Button variant="ghost" size="icon" className="rounded-full hover:bg-[#0066cc]/10" onClick={() => setSelectedSubmission(sub)} disabled={!sub.fileName}><FileSearch className="h-4 w-4 text-[#0066cc]" /></Button></div></TableCell></TableRow>))}</TableBody></Table></div></>)}</CardContent></Card>
        </TabsContent>
      </Tabs>

      <Dialog open={!!selectedSubmission} onOpenChange={o => !o && setSelectedSubmission(null)}>
        <DialogContent className="rounded-[32px] p-8 max-w-2xl bg-white shadow-2xl border-[#d2d2d7]"><DialogHeader><DialogTitle className="text-2xl font-bold text-[#1d1d1f]">Submission Analysis</DialogTitle></DialogHeader>{selectedSubmission && (<div className="space-y-8 py-6"><div className="flex justify-between items-center border-b border-[#d2d2d7] pb-6"><div className="space-y-1"><span className="font-bold text-xl block text-[#1d1d1f]">{selectedSubmission.studentName}</span><span className="text-sm text-[#86868b] font-bold">ID: {selectedSubmission.rollNumber}</span></div><div className="text-right"><span className="text-[#0066cc] font-bold text-4xl">{selectedSubmission.marks}</span><span className="text-[#86868b] text-xl font-bold">/100</span></div></div><div className="space-y-4"><p className="text-xs font-bold uppercase tracking-widest text-[#86868b] flex items-center gap-2"><Sparkles className="h-4 w-4 text-[#0066cc]" /> AI Feedback</p><div className="text-lg leading-relaxed italic p-8 bg-[#f5f5f7] rounded-[24px] border border-[#d2d2d7] text-[#1d1d1f] font-bold">"{selectedSubmission.aiFeedback}"</div></div></div>)}<DialogFooter><Button onClick={() => setSelectedSubmission(null)} className="apple-button-primary w-full font-bold py-6">Done</Button></DialogFooter></DialogContent>
      </Dialog>
    </div>
  );
}
